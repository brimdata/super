...
func NewReader(ctx context.Context, sctx *super.Context, r io.Reader, opts ReaderOpts) (vio.Puller, error) {
	...
	if arrowsErr == nil {
		...
		return newDematerializer(sctx, r), err
	}
	...
	if zeekErr == nil {
		return newDematerializer(sctx, zeekio.NewReader(sctx, track.Reader())), nil
	}
	track.Reset()
	...
	if jsonErr == nil {
		return newDematerializer(sctx, jsonio.NewReader(sctx, track.Reader())), nil
	}
	track.Reset()

	supErr := match(supio.NewReader(super.NewContext(), track), "sup", 1)
	if supErr == nil {
		return newDematerializer(sctx, supio.NewReader(sctx, track.Reader())), nil
	}
	track.Reset()
	...
	if csvErr == nil {
		return newDematerializer(sctx, csvio.NewReader(sctx, track.Reader(), csvio.ReaderOpts{Delim: ','})), nil
	}
	track.Reset()
