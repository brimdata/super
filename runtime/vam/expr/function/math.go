...
func (a *Abs) Call(args ...vector.Any) vector.Any {
	if vec, ok := expr.CheckForErrorThenNull(a.sctx, args, "abs"); ok {
		return vec
	}
...
func (c *Ceil) Call(args ...vector.Any) vector.Any {
	if vec, ok := expr.CheckForErrorThenNull(c.sctx, args, "ceil"); ok {
		return vec
	}
	vec := vector.Under(args[0])
	switch id := vec.Type().ID(); {
...
func (f *Floor) Call(args ...vector.Any) vector.Any {
	if vec, ok := expr.CheckForErrorThenNull(f.sctx, args, "floor"); ok {
		return vec
	}
	vec := vector.Under(args[0])
	switch id := vec.Type().ID(); {
...
func (l *Log) Call(args ...vector.Any) vector.Any {
	if vec, ok := expr.CheckForErrorThenNull(l.sctx, args, "log"); ok {
		return vec
	}
...
func (p *Pow) Call(args ...vector.Any) vector.Any {
	if vec, ok := expr.CheckForErrorThenNull(p.sctx, args, "pow"); ok {
		return vec
	}
...
func (r *Round) Call(args ...vector.Any) vector.Any {
	if vec, ok := expr.CheckForErrorThenNull(r.sctx, args, "round"); ok {
		return vec
	}
	vec := args[0]
	switch id := vec.Type().ID(); {
...
func (s *Sqrt) Call(args ...vector.Any) vector.Any {
	if vec, ok := expr.CheckForErrorThenNull(s.sctx, args, "sqrt"); ok {
		return vec
	}
	vec := vector.Under(args[0])
