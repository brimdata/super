...
func parseMarkdown(source []byte) (map[string]string, []*Test, error) {
	...
	err := ast.Walk(doc, func(n ast.Node, entering bool) (ast.WalkStatus, error) {
		...
		case "mdtest-output":
			...
			for _, s := range fcbInfoWords(commandFCB, source)[1:] {
				...
				case strings.HasPrefix(s, "runtime="):
					commandRuntime = strings.TrimPrefix(s, "runtime=")
					if commandRuntime != "sam" && commandRuntime != "vam" {
						msg := fmt.Sprintf("unknown runtime in mdtest-command info string: %q", commandRuntime)
		...
		case "mdtest-spq":
			var fails bool
			var spqRuntime string
			for _, word := range fcbInfoWords(fcb, source)[1:] {
				switch {
				case word == "fails":
					fails = true
				case strings.HasPrefix(word, "runtime="):
					spqRuntime = strings.TrimPrefix(word, "runtime=")
					if spqRuntime != "sam" && spqRuntime != "vam" {
						msg := fmt.Sprintf("unknown runtime in mdtest-spq info string: %q", spqRuntime)
