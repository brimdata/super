...
func newZoneMapPruner(pred dag.Expr) dag.Expr {
	e, ok := pred.(*dag.BinaryExpr)
	if !ok {
		return nil
	}
	switch e.Op {
	case "and":
		lhs := newZoneMapPruner(e.LHS)
		rhs := newZoneMapPruner(e.RHS)
		if lhs == nil {
			return rhs
		}
		if rhs == nil {
	...
	case "==", "<", "<=", ">", ">=":
		...
		return zoneMapPrunerPred(op, this, literal)
	case "in":
		...
		for _, elem := range elems {
			...
			b := zoneMapPrunerPred("==", this, literal)
			if ret == nil {
				ret = b
			} else {
...
func zoneMapPrunerPred(op string, this *dag.This, literal *dag.Literal) *dag.BinaryExpr {
