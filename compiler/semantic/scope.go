...
func derefSchema(sch schema, seq dag.Seq, name string) (dag.Seq, schema) {
	...
	case *schemaSelect:
		// XXX
		if name == "" {
			name = "unamed_subquery" //XXX postgres and duckdb do this?!
		}
		// XXX currently this is always schemaAnon but this will change so and need
		// to be updated here so this assertion will catch the change.
