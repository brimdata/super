...
// wrapping the result in a named record when needed as by default the data is
// simply a polymorphic sequence.
...
func (a *analyzer) semFromEntity(entity ast.FromEntity, alias *ast.Name, args ast.FromArgs, seq dag.Seq) (dag.Seq, schema) {
	...
	case *ast.SQLPipe:
		//XXX since this is an embedded scope, I think we should hide the schema.
		// it's up to the outer scope to provide the schema
		// e.g., this should fail: select T.a from (from (select * from a1.json) T)
		// while this should work: select T.a from (from (select * from a1.json)) T
		var name string
		if alias != nil {
			name = alias.Text
		}
