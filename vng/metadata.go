...
func ZoneMapValues(zctx *super.Context, m Metadata) []super.Value {
	var b zcode.Builder
	var values []super.Value
	if dynamic, ok := m.(*Dynamic); ok {
		for _, m := range dynamic.Values {
			b.Reset()
			typ := zoneMapValue(zctx, &b, m)
...
func zoneMapValue(zctx *super.Context, b *zcode.Builder, m Metadata) super.Type {
	switch m := Under(m).(type) {
	case *Dict:
		return zoneMapValue(zctx, b, m.Values)
	case *Record:
		var fields []super.Field
		b.BeginContainer()
		for _, f := range m.Fields {
			fields = append(fields, super.Field{Name: f.Name, Type: zoneMapValue(zctx, b, f.Values)})
		}
		b.EndContainer()
		return zctx.MustLookupTypeRecord(fields)
	case *Array:
	...
	case *Primitive:
		...
		return zoneMapLeaf(zctx, b, min, max)
	case *Int:
		return zoneMapLeaf(zctx, b, super.NewInt(m.Typ, m.Min), super.NewInt(m.Typ, m.Max))
	case *Uint:
		return zoneMapLeaf(zctx, b, super.NewUint(m.Typ, m.Min), super.NewUint(m.Typ, m.Max))
	case *Const:
		return zoneMapLeaf(zctx, b, m.Value, m.Value)
	}
	panic(m)
}

func zoneMapLeaf(zctx *super.Context, b *zcode.Builder, min, max super.Value) super.Type {
	b.BeginContainer()
	b.Append(min.Bytes())
	b.Append(max.Bytes())
	b.EndContainer()
