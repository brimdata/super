...
The SuperDB is developed in Go so support for Go clients is
...
the core SuperDB implementation.  We intend to develop a Go package that
...
* [zio](https://pkg.go.dev/github.com/brimdata/super/zio) - I/O interfaces for Super data following the Reader/Writer patterns
...
* [lake/api](https://pkg.go.dev/github.com/brimdata/super/lake/api) - interact with a SuperDB lakehouse
...
This example interacts with a SuperDB lakehouse.  Note that it is straightforward
...
func main() {
	if len(os.Args) != 2 {
		log.Fatalln("URI of lakehouse not provided")
	}
...
error("missing")
```
Note that the order of data has changed because the lakehouse stores data
