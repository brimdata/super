...
A `FROM` clause has the following syntax:
```
FROM <item> [ , <item> ... ]
```
where `<item>` is one or more comma-separated data sources having the form:
```
<file> [ ( format <fmt> ) ] [ <as> ]
<pool> [ @<commit> ] [ <as> ]
...
AS <alias>
```
where `<alias>` is an [identifier](../queries.md#identifiers).
