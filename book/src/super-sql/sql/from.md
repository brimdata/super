...
A `FROM` clause is a component of a [SELECT](select.md) query that
...
in the various expressions that appear throughout the query.
...
```
where `<table-expr>` is one for the forms
```
<file> [ ( format <fmt> ) ] [ <as> ]
<pool> [ @<commit> ] [ <as> ]
<url> [ ( format <fmt> method <method> headers <expr> body <string> ) ] [ <as> ]
eval ( <expr> ) [ <as> ] XXX check as
( <pipe-query> ) [ <as> ]
<named-query> [ <as> ]
```
where `<file>`, `<pool>`, etc. are defined as in the pipe [from](../operators/from.md),
`<name>` is the name of a declared query,
and the optional `<as>` clauses bind an optional table alias to the indicated
item having the syntax:
```
[ AS ] <alias>
```
...
<table> [ ( <column> [ , <column> ... ] ) ]
```
where `<table>` and `<column>` are [identifiers](../queries.md#identifiers)
...
> * allows the binding of table aliases to relational data sources,
> * does not support the [eval](../operators/from.md#expression) form
>   of [from](../operators/from.md), and
...
XXX update below (copied from pipe from)
...
As with pipe [from](../operators/from.md),
optional arguments to `from` may be appended as a parenthesized concatenation
of arguments.  Likewise, automatic format detection for data sources
works as in pipe [from](../operators/from.md).
...
used in combination with other operators like [`fork`](../operators/fork.md) and [`join`](../operators/join.md).
