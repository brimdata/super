...
The error data type conforms to the definition of the
[error type](../../formats/model.md#27-error) in the 
...
error ( <type> )
```
where `<type>` is any SuperSQL type.

Error values can be created with the error function of the form 
```
error ( <value > )
```
where `<value>` is any SuperSQL value.
...
into an error by wrapping it in an [`error` type]().
...
---
_The error type of an corresponding to an error value_
