...
Optional arguments to `from` may be appended as a parenthesized concatenation of named [arguments](#arguments).
...
A text entiy or f-string is an [URL](#urls) if it parses as an URL and otherwise,
...
used an the `<entity>` string argument.  Each such entity is scanned
...
Options to `from` may be appended as a parethesized list of name/value pairs
having the form:
```
...
File-system paths are intepreted relative to the directory in which
