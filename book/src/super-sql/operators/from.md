...
regular expression [`<regexp>`](../search-expressions.md#regular-expressions) or
glob-style pattern [`<glob>`](../search-expressions.md#globs).
...
When the `<url>` argument begin with `http:`, `https:`, or `s3:`
and has the form of a valid URL, then the source is fetched remotely using the
indicated protocol.

As a [text entity](../syntax.md#todo), any valid URL need not be quoted.
...
echo '{a:1}{a:2}' > a.sup
echo '{b:3}{b:4}' > b.sup
echo '"a.sup" "b.sup"' | super -c "from eval(this) | c:=coalesce(a,b)+1" -
```
=>
```mdtest-output
{a:1}
