...
On the other hand, Zeek TSV can be converted to super-structured storage formats and back to
...
mapping from Zeek to super-structured data and back to Zeek again, the sections linked from the
...
out again in the Zeek TSV log format. Other implementations of the super-structured storage
...
these values are represented with a type name bound to the string
...
in SuperDB with a [named type](../../../super-sql/types/named.md) of "port"
bound to the [uint16](../../../super-sql/types/numbers.md) type. See the text
...
The super-structured data model includes a [bytes](../../../super-sql/types/bytes.md) type
that's suited to storing the former "always binary" case and a
[string](../../../super-sql/types/string.md) type for the latter "always printable" case. However, Zeek logs do
not currently communicate details that would allow an implementation to know
which Zeek string fields to store as which of these two data types.
Instead, SuperDB does what the Zeek system does when writing strings
to JSON: any `\x` escapes used in Zeek TSV strings are translated into valid
UTF-8 strings by escaping the backslash before the `x`.  In this way,
you can still see binary-corrupted strings that are generated by Zeek in
the super-structured data formats.
...
solution would be to maintain Zeek strings that are valid UTF-8 as
strings and convert invalid strings into the bytes type, or we could
convert both of them into a [union](../../../super-sql/types/union.md) of string and bytes.  If you have
interest in a capability like this, please [let us know](https://www.brimdata.io/join-slack/) and we can elevate
the priority.

If Zeek were to provide an option to output logs directly in super-structured
storage formats, this would create an opportunity to
assign the appropriate bytes or string type at the point of origin,
...
`id.resp_p`. When reading such fields, SuperDB restores
the hierarchical nature of the record as it originally existed inside of Zeek
itself before it was output by its logging system. This enables queries
that refer to the record at a higher level but affect all values lower
down in the record hierarchy.

For instance, revisiting the data from our example, we can output all fields within
a record using SuperSQL's [cut](../../../super-sql/operators/cut.md) operator.
