...
type conn_id={orig_h:ip,orig_p:port,resp_h:ip,resp_p:port};
```
The `port` and `zenum` types are described further in the [Zeek Type Compatibility](data-type-compatibility.md)
...
> _See [the role of `_path`](reading-zeek-log-formats.md#the-role-of-_path)
